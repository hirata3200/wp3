...
// 			$fields['author'] = '<p class="comment_form_author"><label for="author">お名前</label>' . ($req ? '<span class="required">*</span>' : '' ) . '<input id="author" name="author" type="text" value="' . esc_attr($commenter['comment_author']) .'" size="30" ' .$aria_req . ' /></p>';

// 			$fields['email'] = '<p class="comment_form_email"><label for="email">メールアドレス</label>' . ($req ? '<span class="required">*</span><span class="small">（メールアドレスは公開されません）</span>' : '' ) . '<input id="email" name="email" type="text" value="' . esc_attr($commenter['comment_author_email']) .'" size="30" ' .$aria_req . ' /></p>';
...
// 			$req = get_option( 'require_name_email' );
// 			$required_text = '<span class="required">*</span>が付いてる項目は、必須項目です！';


// 			$form['comment_field'] = '<p class="comment_form_comment"><label for="comment">コメント</label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true"></textarea></p>';

// 			$form['must_log_in'] = '<p class="must_log_in">' . sprintf( 'コメントを残すには、<a href="%s">ログイン</a>してください。', wp_login_url( apply_filters( 'the_permalink', get_permalink( $post->id ) ) ) ) .'</p>';

// 			$form['logged_in_as'] = '<p class="logged_in_as">' . sprintf( '<a href="%1$s">2$s</a>としてログインしています。<a href="%3$s" title="Log out of this account">ログアウト</a>しますか？', admin_url( 'profile.php' ), $user_identity, wp_logout_url( apply_filters( 'the_permalink', get_permalink( $post->id ) ) ) ) .'</p>';
