			...
			if ( is_page() ) :
				if ( $post->post_parent == 0) : /** 親ページの場合 */ ?>
					...
					<?php if ( get_post_meta ( $post->ID, 'sub-title' ) ) : ?>
							<h3><?php echo esc_html( get_post_meta( $post->ID, 'sub-title', true ) ); /** カスタムフィールド 'sub-title' の値を表示 */ ?></h3>
					<?php else : ?>
							<h3>Subtitle</h3>
					<?php endif;
				else : 							/** 子ページの場合 */ ?>
					<h2><?php echo esc_html( get_the_title( $post->post_parent ) ); /** 親ページのタイトルを表示 */ ?></h2>
					<?php if ( get_post_meta( $post->post_parent, 'sub-title' ) ) : ?>
						<h3><?php echo esc_html( get_post_meta( $post->post_parent, 'sub-title', true ) ); /** 親ページのカスタムフィールド 'sub-title' の値を表示 */ ?></h3>
					<?php else: ?>
						<h3>Subtitle</h3>
					<?php endif;
				endif;
